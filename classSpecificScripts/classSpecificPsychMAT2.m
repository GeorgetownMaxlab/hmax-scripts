function c2 = classSpecificPsychMAT2(hmaxHome, faces, scrambled, houses, inverted, configural, empty, ...
...
saveLoc = '/home/bentrans/Documents/HMAX/feature-learning/HMAX-MATLAB/lfwTriples50Levan/';
ext = 'bmp';
...
faces = faces(ceil(length(faces) / 2 + 1:length(faces)));
scrambled = lsDir(scrambled, {ext});
scrambled = scrambled(ceil(length(scrambled) / 2 + 1:length(scrambled)));
houses = lsDir(houses, {ext});
houses = houses(ceil(length(houses) / 2 + 1:length(houses)));
inverted = lsDir(inverted, {ext});
inverted = inverted(ceil(length(inverted) / 2 + 1:length(inverted)));
configural = lsDir(configural, {ext});
configural = configural(ceil(length(configural) / 2 + 1:length(configural)));
empty = lsDir(empty, {ext});
empty = empty(ceil(length(empty) / 2 + 1:length(empty)));
uniqueEmpty = lsDir(uniqueEmpty, {ext});
uniqueEmpty = uniqueEmpty(ceil(length(uniqueEmpty) / 2 + 1:length(uniqueEmpty)));
