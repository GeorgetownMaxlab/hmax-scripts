function [c2,c1,bestBands,bestLocations,s2Layer,s1] = extractC2FromCell(filters,filterSizes,c1Space,c1Scale,c1OL,linearPatches,imgs,...
    nPatchSizes,patchSizes,nPatches,ALLS2C1PRUNE,c1,ORIENTATIONS2C1PRUNE,IGNOREPARTIALS,keepS2,maxSize,RESIZE)
...
%     the maximal response for each patch and image
    ...
    for iImg = 1:nImgs
        iImg
        % At some point, must take the image processing outside of function, preprocess images.
        if RESIZE == 1
		  img = resizeImage(imgs{iImg},maxSize);
        else
          img = imgs{iImg};
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
