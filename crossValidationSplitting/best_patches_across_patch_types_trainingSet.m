function best_patches_across_patch_types_trainingSet(condition)
% This script will look at each patch type, find indices of best patches
% and filter the C2, bestBands, bestLoc, etc files to have only
% those patches.
% clear; clc; close all;
...
saveLoc   = fullfile(home,condition,'data','acrossPatchTypes','lfwSingle50000','fixedLocalization');
if ~exist(saveLoc)
    mkdir(saveLoc)
end
...
for iTask = 1:numel(tasks)
    ...
    sortSumStatsPatch = sortSumStatsPatch(1:nTopPatches);
%     figure
%     plot(sortSumStatsPatch)

    % sort the c2-related matrices by best patches.
    c2f            = c2f           (idx_best_patches,:);
    bestBands      = bestBands     (idx_best_patches,:);
    bestLoc        = bestLoc       (idx_best_patches,:,:);
    imgHitsFaceBox = imgHitsFaceBox(idx_best_patches,:);
    imgHitsWedge.wedgeDegree_90 = imgHitsWedge.wedgeDegree_90(idx_best_patches,:);
    imgHitsWedge.wedgeDegree_45 = imgHitsWedge.wedgeDegree_45(idx_best_patches,:);
    imgHitsWedge.wedgeDegree_30 = imgHitsWedge.wedgeDegree_30(idx_best_patches,:);
    imgHitsWedge.wedgeDegree_20 = imgHitsWedge.wedgeDegree_20(idx_best_patches,:);
    imgHitsWedge.wedgeDegree_15 = imgHitsWedge.wedgeDegree_15(idx_best_patches,:);    
    imgHitsWedge.wedgeDegree_10 = imgHitsWedge.wedgeDegree_10(idx_best_patches,:);    
    
    % Concatenate the matrices
    c2f_big               = [c2f_big;c2f];
    bestBands_big         = [bestBands_big;bestBands];
    bestLoc_big           = [bestLoc_big;bestLoc];
    sumStatsPatch_big     = [sumStatsPatch_big;sortSumStatsPatch]; % the big file is called sumStatsPatch_big instead of sortSumStatsPatch_big. Because even though, for each individual patch-type the patches are sorted, when we concatenate their sorted performances the combined performances are no longer sorted.
...
%% Take the new, combined matrix. Sort it by the combined sortSumStats
% % variable. So the new matrix will be sorted from best to worst.
% [meta_sortSumStatsPatch_big,meta_idx_best_patches_big] = sort(sumStatsPatch_big,'descend');
% 
...
% 
% sortSumStatsPatch = meta_sortSumStatsPatch_big;
% idx_best_patches  = 1:size(c2f,1); % since patches are sorted, the idx of best ones will simply be progression from first to last.
% patchSizes        = patchSizes(meta_idx_best_patches_big);

%% DO NOT SORT THIS NEW COMBINED DATA SET. Just sort its 
...
%% Save everything
% save(fullfile(saveLoc,'meta_idx_best_patches_big.mat'),'meta_idx_best_patches_big');
...
save(fullfile(saveLoc,'bestLocC2f.mat'),'bestLoc','-v7.3');
save(fullfile(saveLoc,'imgHitsFaceBox.mat'),'imgHitsFaceBox');
