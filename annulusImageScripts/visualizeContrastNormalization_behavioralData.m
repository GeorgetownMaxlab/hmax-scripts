...
% folder
% .../HMAX/annulusExptFixedContrast/simulation1/testing/behavioral_data
% these are the images that actual subjects saw

% LB 2017-02-10: this script is basically the same as
% CUR_visualizeContrastNormalization2.m so why not use that?...
...
load('C:\Users\levan\HMAX\annulusExptFixedContrast\simulation1\testing\behavioral_data/exptDesign.mat')

%% Using only exptDesign variable

loadLoc = 'C:\Users\levan\HMAX\annulusExptFixedContrast\simulation1\testing\behavioral_data/images';
...
for iTrial = 1:length(faceImg) %used to be "length(mask)"
    ...
    allMasks_resized{iTrial} = resizeImage(allMasks{iTrial},maxSize);
    iMask                    = allMasks_resized{iTrial};
%     assert(isequal(size(iFaceImg),size(allMasks_resized{iTrial})),'Mask and Face Image Dimensions Don''t Match!!!')
    dimensionCheck(iTrial)   = isequal(size(iFaceImg),size(allMasks_resized{iTrial}));
%     iImage = double(imread(imgPaths_natsort{iImg}));
%     idx2 = iTrialMask_res > 0;
    idx                      = find(allMasks_resized{iTrial} > 50);
    
    % double check that indexin worked
    newFace = zeros(size(iFaceImg));
    newFace(idx) = 1;
    imwrite(newFace,...
        ['C:\Users\levan\HMAX\annulusExptFixedContrast\simulation1\testing\behavioral_data/sandbox\'...
        int2str(iTrial) '.png']);
    
    % Get contrast
   max_vignette(iTrial)     = max(iFaceImg(idx));
   min_vignette(iTrial)     = min(iFaceImg(idx));   
...
save('exptDesign.mat','quadrant','positionAngle','position','michelson_values',...
    'faceName','faceImg','bgName','allMasks_resized')
