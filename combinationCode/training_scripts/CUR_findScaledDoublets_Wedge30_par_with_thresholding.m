function CUR_findScaledDoublets_Wedge30_par_with_thresholding(...
        nPatchesAnalyzed,...
        iPatchLoop,...
        nTPatchesPerLoop,...
        nCPatches,...
...
%     idx_for_combMatrix
    ...
    % Check that the complementary patch does at least somewhat well overall
    which_CPatches_Survive = ismember(idxCPatches,cPatchesSurviving);
    % out of the nCPatches, how many were eliminated?
    idxCPatchesTemp = idxCPatches(1:nCPatches);
    which_eliminated = ~ismember(idxCPatchesTemp,cPatchesSurviving);
    display(['Thresholding eliminated ' int2str(nnz(which_eliminated)) ' CPatches']);
    % Filter out low-performing patches    
    sortSumStats_CPatch    = sortSumStats_CPatch(which_CPatches_Survive);
    idxCPatches            = idxCPatches        (which_CPatches_Survive);
