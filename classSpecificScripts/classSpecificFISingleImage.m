...
%tests a single image (one that gave the highest FI, extracting all possible patches from it) to see if 
...
training = lsDir('/home/bentrans/Documents/2.97/', {'JPEG'});
testing = targets;
...
%extract patches
	...
	[patches, locations, bands] = extractAllPatches(c1r, [n * 2; n * 2; 4]);
	ps.patches = patches;
	ps.locations = locations;
	ps.bands = bands;
	save(['ps' num2str(n)], 'ps');

%START SECOND STAGE
	fprintf('converting patches to xml... \n');
	patchFile = matlabPatches2OCVPatches(filters, fSize, c1Scale, c1Space, c1OL, {ps.patches'}, [n * 2; n * 2;4;length(ps.patches)] , 'patches', hmaxHome);
