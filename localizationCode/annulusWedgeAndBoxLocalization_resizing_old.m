function CUR_annulusWedgeAndBoxLocalization_resizing(nPatches, loadFolder, condition, ...
    runImgDiff, maxSize, facesFileToLoad, saveFolder, quadType, patchSize)

% This script was discovered to have a bug. It only used the first image in
% the series to determine scaling factors.

%% Define global variables and load files.

if(nargin < 1)
    nPatches = 100;
    loadFolder = 'eeg_up_inv\upright\data\patchSetAdam\lfwSingle50000';
    condition = 'eeg_up_inv';
    saveFolder = [loadFolder '/sandboxNew'];
end
...
if ispc
    pcStatus = 1;
    home = ['C:/Users/Levan/HMAX/' condition '/']; % location for exptDesign file
    loadLoc = ['C:/Users/Levan/HMAX/' loadFolder '/']
    load([loadLoc facesFileToLoad]); %variable in workspace called 'facesLoc']
    saveLoc = ['C:/Users/Levan/HMAX/' saveFolder '/']
else
    pcStatus = 0;
    home = ['/home/levan/HMAX/' condition '/']; % location for exptDesign file
    loadLoc = ['/home/levan/HMAX/' loadFolder '/']
    load([loadLoc facesFileToLoad]) %variable in workspace called 'facesLoc'
...
mkdir([saveLoc 'responseOverlays/hits']);
mkdir([saveLoc 'responseOverlays/misses']);
...
assert(isequal(round(scalingFactor,2),round(xSizeOrig/xSizeResized,2)),'somethings not right with image resizing');
...
%     for iImg = 1:size(facesLoc{1},2)
%         key = '_image';
%         key2 = '_faceName';
%         idx  = strfind(facesLoc{1}{iImg},key);
%         idx2 = strfind(facesLoc{1}{iImg},key2);
%         imgFacesLoc(iImg) = str2double(facesLoc{1}{iImg}((idx+length(key)):idx2-1));
%     end
%     sanityCheck.imgFacesLoc = imgFacesLoc;
...
for iPatch = 1:nPatches %fprintf('STARTING W CUSTOM PATCH!!')
    ...
    for iImg = 1:size(bestBands,2) %fprintf('STARTING W CUSTOM IMAGE!!')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        faceBoxOrig = [position{imgFacesLoc(iImg)}(2)-50 ...
                       position{imgFacesLoc(iImg)}(2)+50 ...
                       position{imgFacesLoc(iImg)}(1)-50 ...
                       position{imgFacesLoc(iImg)}(1)+50]; %[x1 x2 y1 y2]
