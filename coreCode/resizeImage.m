...
%   aspect ratio of the original image
    imgOut = double(imresize(imgIn,maxSize/max(size(imgIn)))); %allows you to resize larger than original maxSize
%	 imgOut = double(imresize(imgIn, min(1, maxSize / max(size(imgIn))))); %allows you to only resize the image smaller
end
