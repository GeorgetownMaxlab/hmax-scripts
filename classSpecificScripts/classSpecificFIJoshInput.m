...
%uses Josh's distractors, test images, training images, and preexisting patches
%
...
trainingLoc = load('patchImgs.mat');
trainingLoc = trainingLoc.patchImgs;
for n = 1:length(trainingLoc)
	[a, b, c] = fileparts(trainingLoc{n});
	trainingLoc{n} = [targets b '.JPEG'];
end
training = trainingLoc;
dLoc = load('experimentalImgs');
dLoc = dLoc.imgLists.experimentalImgs;
for n = 1:length(dLoc)
	[a, b, c] = fileparts(dLoc{n});
	dLoc{n} = [distractors b '.jpg'];
end
testingLoc = load('testing.mat');
testingLoc = testingLoc.experimentalImgs;
...
save('c1Josh', 'c1r');
save('s1Josh', 's1r'); %not working due to some matlab version error, not important anyways
%c1r = load('c1.mat');
%c1r = c1r.c1r;
...
%	save(['patchJosh' int2str(n)], 'patches'); 
