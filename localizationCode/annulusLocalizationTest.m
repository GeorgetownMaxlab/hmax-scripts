function hits = annulusLocalizationTest(facesFileToLoad, nPatches, loadFolder, ...
...
% startFace = str2double(startFace);
% endFace   = str2double(endFace);
            ...
            imgPerCell = size(bestLoc{n}(iPatch, :, :), 2);
                for iImg = 1:imgPerCell
                    ...
                    imgHits{1}{n}{i}{iImg} = 0;
                        if(y2p > loc{1}(3) && y1p < loc{1}(4) && x1p < loc{1}(2) && x2p > loc{1}(1)) %if best match is within where the face is
...
save([saveLoc 'hits' startFace '-' endFace], 'hits');
save([saveLoc 'sanityCheck' startFace '-' endFace], 'sanityCheck');

concatinateImgHitsAnnulusTest(imgHits,quadType,nPatches,loadLoc,saveLoc,startFace,endFace);
