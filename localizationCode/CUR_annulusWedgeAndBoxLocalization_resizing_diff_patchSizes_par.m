function CUR_annulusWedgeAndBoxLocalization_resizing_diff_patchSizes_par(...
                    iPatchLoop,...
                    nPatchesPerLoop,...
                    patchSizes,...
                    loadLoc,...
...
for iPatch = 1:nPatchesPerLoop %fprintf('STARTING W CUSTOM PATCH!!')
%     if mod(iPatch,500) == 0
...
% sanityCheckPatchLoop.dist_in_deg2(iPatch,iImg) = dist_in_deg2;
% sanityCheckPatchLoop.dist_in_rad2(iPatch,iImg) = dist_in_rad2;
...
% criterionInDeg = [45,30,15,10];
% criterionInRad = deg2rad(criterionInDeg);
% 
% 
% 
% for iCrit = 1:numel(criterionInRad)
%     criterionDistancesRad = ones(nPatchesPerLoop,nImgsAnalyzed)*criterionInRad(iCrit); % not efficient. Cause is being generated for each loop.
%     criterionDistancesDeg = ones(nPatchesPerLoop,nImgsAnalyzed)*criterionInDeg(iCrit);
%     imgHitsWedgePatchLoopRad{iCrit} = criterionDistancesRad > angularDistPatchLoopRad;
%     imgHitsWedgePatchLoopDeg{iCrit} = criterionDistancesDeg > angularDistPatchLoopDeg;
%     assert(isequal(imgHitsWedgePatchLoopRad{iCrit},imgHitsWedgePatchLoopDeg{iCrit}));
% %     figure
% %     imagesc(imgHitsWedgePatchLoopRad{iCrit})
% end
...
%%
% % Plot differences between different ways of calculating the angular distance.
% figure
% subplot(1,2,1)
% A = diff(sanityCheckPatchLoop.dist_in_rad1 - sanityCheckPatchLoop.dist_in_rad2);
% max(abs(A(:)))
% imagesc(A);
% subplot(1,2,2)
% B = diff(sanityCheckPatchLoop.dist_in_deg1 - sanityCheckPatchLoop.dist_in_deg2);
% max(abs(B(:)))
% imagesc(diff(sanityCheckPatchLoop.dist_in_deg1 - sanityCheckPatchLoop.dist_in_deg2));
% suptitle(['Patches ' int2str(patchSizes(1)) 'x' int2str(patchSizes(2))]);
...
save(fullfile(saveLoc,['imgHitsWedge_'   int2str(idxPatchStart) '-' int2str(idxPatchEnd) '_allImages']),  'imgHitsWedgePatchLoop');
