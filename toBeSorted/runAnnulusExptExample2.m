function c2 = runAnnulusExptExample2(locationFileToLoad, loadFolder, ...
...
if (nargin < 1)
    locationFileToLoad  = 'sourceImages.mat';
    loadFolder          = 'not_needed';
    saveFolder          = 'sandbox\our';
    quadType            = 'f';
    nPatchesAnalyzed    = 3;
    idxFaceStart        = 1;
    idxFaceEnd          = 2;
...
if ispc == 1 
    loadLoc         = 'C:\Users\Levan\HMAX\testingHMAX\'
    saveLoc         = ['C:\Users\Levan\HMAX\testingHMAX\' saveFolder '\']
    patchesLoc      = ('C:\Users\Levan\HMAX\testingHMAX\')
...
% If loading and testing all of the patches
    load([patchesLoc 'joshPatches2.mat']);
%     patches = ps;
    patches.patches = patches;
    patches.sizes   = patchSizes;
    for iPatchSizes = 1:size(patchSizes,2)
        patches.patches{iPatchSizes} = patches.patches{iPatchSizes}(:,1:nPatchesAnalyzed);
    end
    fprintf('loaded the patches\n');
%% Save parameter space.%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
runDateTime = datetime('now');
outputOfPWD = pwd;
runParameterComments = 'this is a run only for debugging' %input('Any comments about run?\n');
...
%% matlab implementation to create C2    
    if strcmp(quadType,'f') == 1
        ...
        for i = 1:length(facesLoc)
            [c2f, ~, bestBands, bestLoc, s2f] = genC2Test(gaborSpecs, facesLoc{i}, c1bands, patches.patches, patches.sizes, 1, maxSize, COMBINATION, BLUR, keepS2, RESIZE);
        ...
        for i = 1:length(emptyLoc)
            [c2e, ~, bestBands, bestLoc, s2e] = genC2Test(gaborSpecs, emptyLoc{i}, c1bands, patches.patches, patches.sizes, 1, maxSize, COMBINATION, BLUR, keepS2, RESIZE);
