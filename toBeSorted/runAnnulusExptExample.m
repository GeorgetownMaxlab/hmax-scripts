function c2 = runAnnulusExptExample(locationFileToLoad, loadFolder, ...
...
if (nargin < 1)
    locationFileToLoad  = 'noAnnulusRuns\trainingTestingSplit\facesLocTrainingWin.mat';
    loadFolder          = 'not_needed';
    saveFolder          = 'sandbox\exampleRuns\';
    quadType            = 'f';
    nPatchesAnalyzed    = 3;
    idxFaceStart        = 1;
    idxFaceEnd          = 2;
...
%% matlab implementation to create C2    
    if strcmp(quadType,'f') == 1
        ...
        for i = 1:length(facesLoc)
            [c2f, ~, bestBands, bestLoc, s2f] = genC2Test(gaborSpecs, facesLoc{i}, c1bands, patch, patches.sizes, 1, maxSize, COMBINATION, BLUR, keepS2, RESIZE);
        ...
        for i = 1:length(emptyLoc)
            [c2e, ~, bestBands, bestLoc, s2e] = genC2Test(gaborSpecs, emptyLoc{i}, c1bands, patch, patches.sizes, 1, maxSize, COMBINATION, BLUR, keepS2, RESIZE);
