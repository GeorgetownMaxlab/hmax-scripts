function [total_imgs, total_testing_imgs_across_sets] = generate_crossvalid_files_only_images(crossValidInfo)
...
%         load(fullfile(home,imgSets{2,iSet},imgSets{1,iSet},'data',tasks{iTask},'lfwSingle50000','bestLocC2f.mat'));
        ...
        % Sum up the total unmber of training images we'd get.
        eval(['total_imgs = total_imgs + numel(idx_training_images_' imgSets{1,iSet} ');']);
        eval(['total_testing_imgs_across_sets = total_testing_imgs_across_sets + numel(idx_testing_images_' imgSets{1,iSet} ');']);
...
%     
%     saveLoc_training = fullfile(home,'simulation3','training','data',tasks{iTask},'lfwSingle50000');
%     if ~exist(saveLoc_training)
%         mkdir(saveLoc_training)
%     end
%     
%     save(fullfile(saveLoc_training,'c2f.mat'),      'c2f');
%     save(fullfile(saveLoc_training,'bestBands.mat'),'bestBands');
%     save(fullfile(saveLoc_training,'bestLoc.mat'),  'bestLoc');
%     save(fullfile(saveLoc_training,'facesLoc.mat'), 'facesLoc');
%     save(fullfile(home,'simulation3','training','exptDesign.mat'), 'exptDesign');
...
%     
%     saveLoc_testing = fullfile(home,'simulation3','testing','data',tasks{iTask},'lfwSingle50000');
%     if ~exist(saveLoc_testing)
%         mkdir(saveLoc_testing)
%     end
%     
%     save(fullfile(saveLoc_testing,'c2f.mat'),      'c2f');
%     save(fullfile(saveLoc_testing,'bestBands.mat'),'bestBands');
%     save(fullfile(saveLoc_testing,'bestLoc.mat'),  'bestLoc');
%     save(fullfile(saveLoc_testing,'facesLoc.mat'), 'facesLoc');
%     save(fullfile(home,'simulation3','testing','exptDesign.mat'), 'exptDesign');
%     
