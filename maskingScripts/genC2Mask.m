function [c2, c1, bestBands, bestLocations, s2] = genC2Mask(gaborSpecs,imgNames,c1bands,linPatches,patchSpecs, ...
  USEMATLAB, maxSize, keepS2, RESIZE, nImgsPerWorker,idxPixel)
...
% c2: nPatches x nImgs array, C2 responses for an image set
    ...
    for iSize = 1:nPatchSizes
	 ...
	 nPatches = length(patches) / 4; % whats up with this??
...
%         fprintf('%d: start: %d stop: %d\n',iImg,iStart, iStop);
        ...
        if USEMATLAB
            [c, c11, bestBand, bestLoc, s2] = extractC2FromCellMask(linFilters,filterSizes,...
                                   c1bands.c1Space,c1bands.c1Scale,...
                                   c1OL,linPatches,imgs,...
                                   nPatchSizes,...
                                   patchSizes,nPatches,0,[],0,0,keepS2,maxSize,RESIZE,idxPixel); %17
